...
import styles from "./RegionOperatorsPopup.module.css";
...
const RegionOperatorsPopup = ({ isOpen, onClose, dateRange, selectedRegion }) => {
  ...
  // Мемоизированная функция загрузки данных
   const fetchOperatorsData = useCallback(async () => {
        if (!selectedRegion || !currentTable) { // Добавлена проверка на currentTable
            setError("Регион не выбран или таблица не загружена");
            return;
        }
        ...
        try {
            ...
            console.log('📊 Получены данные операторов для региона:', selectedRegion, data);
            ...
            console.error('Ошибка загрузки операторов региона:', err);
            setError(err.message);
        } finally {
            setLoading(false);
        }
    }, [activeTab, dateRange, selectedRegion, tableVersion, currentTable]);

  // Загружаем данные операторов при открытии или смене таба
 useEffect(() => {
        if (isOpen && selectedRegion && currentTable) { // Добавлена проверка на currentTable
            fetchOperatorsData();
        }
    }, [isOpen, selectedRegion, fetchOperatorsData, currentTable]);
  ...
  const tabTitle = useMemo(() => {
    ...
    return tab ? `Операторы: ${tab.name} - ${selectedRegion}` : `Операторы региона: ${selectedRegion}`;
  ...
  const totalStats = useMemo(() => {
    ...
    // Взвешенное среднее по количеству полетов
      ...
      <div className={styles.popup} ref={popupRef}>
        ...
        {loading && (
          <div className={styles.loading}>
            ...
            <p>Загрузка данных операторов...</p>
          </div>
        )}
          ...
          <div className={styles.content}>
            ...
            <div className={styles.tabStats}>
              ...
              <div className={styles.statItem}>
                ...
                <div className={styles.statLabel}>Регион</div>
              </div>
            </div>
              ...
              <div className={styles.operatorsContainer}>
                {operatorsData.operators && operatorsData.operators.length > 0 ? (
                  ...
                  <div className={styles.noData}>
                    Нет данных об операторах для региона {selectedRegion}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className={styles.filtersInfo}>
              ...
              {dateRange?.date_from && dateRange?.date_to && (
                <div className={styles.filterItem}>
                  Период: {dateRange.date_from} - {dateRange.date_to}
                </div>
              )}
            </div>
          </div>
...
export default RegionOperatorsPopup;