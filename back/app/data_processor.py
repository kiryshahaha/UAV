...
class DataProcessor:
    ...
    def save_to_table_with_id(self, df, table_name=None):
        """
        Загрузка данных в таблицу с добавлением уникального ID
        """
        try:
            # ЕСЛИ table_name не указан, используем из .env
            if table_name is None:
                table_name = os.getenv('TABLE_NAME', 'excel_data_2025')
                
            self.logger.info(f" Сохранение {len(df)} строк в таблицу {table_name}")
            ...
            try:
                ...
                # to_sql автоматически создаст таблицу если её нет
                df_cleaned.to_sql(
                    table_name,
                    self.engine,
                    if_exists='replace',  # Перезаписываем таблицу
                    index=False,
                    dtype=dtypes,
                    chunksize=1000
                )
                ...
                if 'id' not in columns_after:
                    ...
                    """))
                    self.logger.info(" Добавлен PRIMARY KEY id")
                
                connection.commit()
                
                added_count = len(df_cleaned)
                self.logger.info(f" Данные сохранены в {table_name}: {added_count} записей")
    ...
    def append_to_table(self, df: pd.DataFrame, table_name: str = "excel_data_result_1") -> dict:
        """
        Добавление данных в существующую таблицу
        """
        try:
            self.logger.info(f"Добавление {len(df)} строк в таблицу {table_name}")

            # Очищаем DataFrame перед сохранением
            df_cleaned = DataProcessor.clean_dataframe(df)
            if df_cleaned.empty:
                self.logger.info("Нет данных для добавления")
                return {"added": 0, "total": 0}
            ...
            try:
                # Получаем максимальный ID для определения новых записей
                max_id_result = connection.execute(text(f"SELECT COALESCE(MAX(id), 0) FROM {table_name}"))
                ...
                df_cleaned.to_sql(
                    table_name,
                    self.engine,
                    if_exists='append',  #  Ключевое изменение - append вместо replace
                    index=False,
                    chunksize=1000
                )
                
                # Получаем количество добавленных записей
                new_max_id_result = connection.execute(text(f"SELECT COALESCE(MAX(id), 0) FROM {table_name}"))
                new_max_id = new_max_id_result.scalar() or 0
                
                added_count = new_max_id - max_id
                
                self.logger.info(f"Добавлено {added_count} записей в таблицу {table_name}")
            ...
            self.logger.error(f"Ошибка при добавлении в PostgreSQL: {e}")
            raise
        except Exception as e:
            self.logger.error(f"Неожиданная ошибка при добавлении данных: {e}")
